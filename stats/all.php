...
exec('ls -t ~/stats.atf2.org/logs/ | grep -v \'L.*[0-9]\.log\' | head -n100', $latestFiles);
foreach($latestFiles as $fileName){
    ...
    if(count($explodedFileName) == 2 && $explodedFileName[1] == 'log' && filesize('./logs/' . $fileName) > 20000){
        exec('grep \' tv_record \' ~/stats.atf2.org/logs/' . $fileName, $mapName);
        $mapName = explode(" ", $mapName[0]);
        $mapName = explode("_", $mapName[count($mapName) - 1]);
        $logDate = $mapName[0];
        $mapName = str_replace('.dem', '', $mapName[4]);
        if($mapName){
            ...
            echo "$newFileName\n";
            echo "SELECT * FROM files WHERE fileName = '$newFileName'\n";
            var_dump(!$database->queryAssoc("SELECT * FROM files WHERE fileName = '$newFileName'"));
